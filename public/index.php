...
// 1) If NOT logged in → only allow login page
if (!$user && $page !== 'login') {
...
// 3) Optional: admin-only pages
$adminOnlyPages = ['products', 'members', 'reports']; // you can customize
if ($user && ($user['role'] ?? '') !== 'admin' && in_array($page, $adminOnlyPages)) {
    // Block access and show a friendly message
    $page = 'home';
...
<body>
  ...
  <script>
    ...
    // helper to show modal from any page
